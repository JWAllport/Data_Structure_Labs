...
//If the node has a different keyword, keep calculating a new hash index until either the keyword matche the node at that index's keyword, or until the
...
//if c1 is true, the first collision method is used, and if it’s false, the second is used
    ...
    int collHash1(int h, int count, string k); // getting index with collision method 1 (note – you may modify the
